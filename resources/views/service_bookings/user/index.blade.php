                    ...
                    <div class="row g-3 align-items-center">
                        ...
                        @if ($booking->status === 'pending')
                            <div class="col-12 col-lg-4 d-flex justify-content-end gap-2">
                                <form action="{{ route('user.bookings.cancel', ['booking' => $booking->id]) }}" method="POST" class="cancel-form">
                                    @csrf
                                    @method('put')
                            ...
                            <div class="card border shadow-none radius-10 h-100 mb-0">
                                <div class="card-body">
                                    <div class="d-flex gap-3">
                                        ...
                                        <div class="info">
                                            <h6 class="mb-2">{{ $booking->staff->name }}</h6>
                                            <p class="mb-1"><strong>{{ $booking->staff->job->job->name }}</strong> ({{ $settings['country_dialing_code'] . ' ' . $booking->staff->phone }})</p>
                                            <p class="mb-1"><strong>Charge</strong> : {{ $settings['currency_symbol'] . $booking->staff->job->service_charge }}/Hour</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
